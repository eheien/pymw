...
parser.add_option("-m", "--num_individuals", dest="n_inds", default="100", help="total number of individuals for genetic algorithm", metavar="DIR")
parser.add_option("-l", "--gene_length", dest="g_len", default="100", help="gene length of each individual", metavar="DIR")
parser.add_option("-s", "--sub_generations", dest="sub_gens", default="10", help="number of generations to run between gene exchanges", metavar="DIR")
...
gene_pool = [[random.randint(0,1) for i in range(gene_len)] for n in range(int(1.3*n_workers*num_inds))]

max_fitness = 0

for q in range(n_workers):
    ind_set = random.sample(gene_pool, inds_per_worker)
